...
import { ArrowLeft, Plus, Minus, ShoppingCart, User, Phone, Mail, Calendar, Clock, Trash2, Star, AlertTriangle, Gift } from 'lucide-react';
import { useAuth } from '../hooks/useAuth';
import { useBooking } from '../hooks/useBooking';
interface PasteMandorlaItem {
  id: string;
  type: string;
  weight: number;
  packaging: string;
  travelPackaging: boolean;
  ribbon: string;
  quantity: number;
}

interface FormData {
  items: PasteMandorlaItem[];
  name: string;
  phone: string;
  email: string;
  pickupDate: string;
...
export default function PasteMandorlaBooking() {
  const { user } = useAuth();
  ...
  const [formData, setFormData] = useState<FormData>({
    items: [{ id: '1', type: '', weight: 250, packaging: '', travelPackaging: false, ribbon: '', quantity: 1 }],
    name: '',
    phone: '',
    email: '',
    pickupDate: '',
  ...
  const calculateTotal = () => {
    return formData.items.reduce((total, item) => {
      if (item.type && item.weight && item.quantity) {
        const weightInKg = item.weight / 1000;
        return total + (weightInKg * item.quantity * 22); // €22 al kg
      }
      return total;
    }, 0);
  };
  ...
  const addItem = () => {
    ...
    setFormData(prev => ({
      ...prev,
      items: [...prev.items, { id: newId, type: '', weight: 250, packaging: '', travelPackaging: false, ribbon: '', quantity: 1 }]
    }));
  };
  ...
  const updateItem = (id: string, field: keyof PasteMandorlaItem, value: any) => {
    setFormData(prev => ({
      ...prev,
  ...
  const validateForm = () => {
    ...
    const validItems = formData.items.filter(item => item.type && item.weight && item.quantity > 0);
    if (validItems.length === 0) {
      newErrors.items = 'Seleziona almeno un tipo di paste di mandorla con peso e quantità';
    }
              ...
              <div className="text-left space-y-3 text-gray-700">
                {formData.items.filter(item => item.type && item.quantity > 0).map((item, index) => (
                  <div key={index} className="border-b border-blue-200 pb-2">
                    <p><strong>{item.quantity}x {item.type} ({item.weight}g)</strong></p>
                    {item.packaging && <p className="text-sm">Confezione: {item.packaging}</p>}
                    {item.travelPackaging && <p className="text-sm">Confezione da viaggio: Sì</p>}
                    {item.ribbon && item.ribbon !== 'Nessun nastro' && (
                      <p className="text-sm">Nastro: {item.ribbon}</p>
                    )}
                  </div>
                ))}
                <p className="pt-2 text-lg"><strong>Peso totale:</strong> {getTotalWeight()}g</p>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-blue-900 mb-6 font-serif">
              PRENOTA LE TUE
              <span className="block text-yellow-600">PASTE DI MANDORLA</span>
            </h1>
            <div className="w-24 h-1 bg-yellow-600 mx-auto mb-8" />
            <p className="text-xl text-gray-700 leading-relaxed mb-8 max-w-3xl mx-auto">
              Ordina le nostre tradizionali paste di mandorla pugliesi, preparate con mandorle 
              selezionate e personalizzabili per ogni occasione.
            </p>
            
            <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-lg mb-8 max-w-2xl mx-auto">
              <p className="text-blue-800 text-sm font-medium">
                <Gift className="w-4 h-4 inline mr-2" />
                Per le paste di mandorla per le tue cerimonie vieni a trovarci in negozio
              </p>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl p-8 md:p-12">
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-semibold text-blue-900 flex items-center">
                    ...
                    Seleziona le tue paste di mandorla
                  </h3>
                  <div className="text-sm text-gray-600">
                    Peso totale: <span className="font-semibold text-blue-900">{getTotalWeight()}g</span>
                  </div>
                </div>
                    ...
                    <div key={item.id} className="border-2 border-gray-200 rounded-lg p-6 relative">
                      ...
                      <div className="grid md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                          ...
                          <select
                            value={item.ribbon}
                            onChange={(e) => updateItem(item.id, 'ribbon', e.target.value)}
                            className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-yellow-400 focus:outline-none transition-colors"
                          >
                            <option value="">Seleziona nastro</option>
                      ...
                      <div className="mb-4">
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            checked={item.travelPackaging}
                            onChange={(e) => updateItem(item.id, 'travelPackaging', e.target.checked)}
                            className="w-4 h-4 text-yellow-600 mr-3"
                          />
                          <span className="text-gray-700">Confezione da viaggio</span>
                        </label>
                      </div>
                      ...
                      {item.type && (
                        <div className="bg-blue-50 p-3 rounded-lg">
                          <p className="text-blue-900 font-medium text-sm">
                            {item.quantity}x {item.type} ({item.weight}g) - €{((item.weight / 1000) * item.quantity * 22).toFixed(2)}
                            {item.packaging && ` - Confezione: ${item.packaging}`}
                            {item.travelPackaging && ` - Da viaggio`}
                            {item.ribbon && item.ribbon !== 'Nessun nastro' && ` - ${item.ribbon}`}
                          </p>
                        </div>
                      )}
                    </div>
                ...
                >
                  ...
                  Aggiungi altro tipo di paste di mandorla
                </button>
              </div>
              ...
              <div className="mb-8">
                ...
                <p className="text-sm text-gray-600 mt-2">
                  * Le paste di mandorla devono essere ordinate con almeno 24 ore di anticipo
                </p>
              </div>
              ...
              <div className="mb-8">
                ...
                <textarea
                  ...
                  placeholder="Richieste particolari, personalizzazioni, allergie..."
