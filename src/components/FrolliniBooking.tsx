...
import { ArrowLeft, Plus, Minus, ShoppingCart, User, Phone, Mail, Calendar, Clock, Trash2, Cookie, AlertTriangle } from 'lucide-react';
import { useAuth } from '../hooks/useAuth';
import { useBooking } from '../hooks/useBooking';
interface FrolliniItem {
  id: string;
  type: string;
  format: number;
  quantity: number;
...
function FrolliniBooking() {
  ...
  const [formData, setFormData] = useState<FormData>({
    items: [{ id: '1', type: '', format: 6, quantity: 1 }],
    name: '',
    phone: '',
    email: '',
    pickupDate: '',
  ...
  const frolliniTypes = [
    'Bagnati al cioccolato',
    'Ripieni di Nutella',
    'Ripieni di albicocca',
    'Ripieni di frutti di bosco',
  ...
  const formatOptions = [6, 12, 24, 30, 40, 50, 60];

  const calculateTotal = () => {
    return formData.items.reduce((total, item) => {
      if (item.type && item.format && item.quantity) {
        return total + (item.format * item.quantity * 0.70); // €0.70 per pezzo
      }
      return total;
    }, 0);
  };
  ...
  const addItem = () => {
    ...
    setFormData(prev => ({
      ...prev,
      items: [...prev.items, { id: newId, type: '', format: 6, quantity: 1 }]
    }));
  };
  ...
  const updateItem = (id: string, field: keyof FrolliniItem, value: any) => {
    setFormData(prev => ({
      ...prev,
  ...
  const getTotalItems = () => {
    return formData.items.reduce((total, item) => {
      if (item.type && item.format && item.quantity) {
        return total + (item.format * item.quantity);
      }
      return total;
    }, 0);
  };
  ...
  const validateForm = () => {
    ...
    const validItems = formData.items.filter(item => item.type && item.format && item.quantity > 0);
    if (validItems.length === 0) {
      newErrors.items = 'Seleziona almeno un tipo di frollini con formato e quantità';
    }
              ...
              <div className="text-left space-y-3 text-gray-700">
                {formData.items.filter(item => item.type && item.quantity > 0).map((item, index) => (
                  <div key={index} className="border-b border-blue-200 pb-2">
                    <p><strong>{item.quantity}x Confezione {item.type} ({item.format} pezzi)</strong></p>
                    <p className="text-sm">Totale pezzi: {item.format * item.quantity}</p>
                  </div>
                ))}
                <p className="pt-2 text-lg"><strong>Totale pezzi ordinati:</strong> {getTotalItems()}</p>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-blue-900 mb-6 font-serif">
              PRENOTA I TUOI
              <span className="block text-yellow-600">FROLLINI ARTIGIANALI</span>
            </h1>
            <div className="w-24 h-1 bg-yellow-600 mx-auto mb-8" />
            <p className="text-xl text-gray-700 leading-relaxed mb-8 max-w-3xl mx-auto">
              Ordina i nostri deliziosi frollini artigianali. Scegli tra diverse varietà 
              e formati per ogni occasione.
            </p>
          </div>
        </div>
      </section>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl p-8 md:p-12">
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-semibold text-blue-900 flex items-center">
                    ...
                    Seleziona i tuoi frollini
                  </h3>
                  <div className="text-sm text-gray-600">
                    Totale: <span className="font-semibold text-blue-900">{getTotalItems()} pezzi</span>
                  </div>
                </div>
                      ...
                      <div className="grid md:grid-cols-3 gap-6">
                        <div>
                          <label className="block text-gray-700 font-medium mb-2">
                            Tipo Frollini <span className="text-red-500">*</span>
                          </label>
                          <select
                            value={item.type}
                        ...
                        <div>
                          <label className="block text-gray-700 font-medium mb-2">
                            Formato Confezione <span className="text-red-500">*</span>
                          </label>
                          <select
                            value={item.format}
                            onChange={(e) => updateItem(item.id, 'format', parseInt(e.target.value))}
                            className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-yellow-400 focus:outline-none transition-colors"
                          >
                            {formatOptions.map(format => (
                              <option key={format} value={format}>{format} pezzi</option>
                            ))}
                          </select>
                        </div>
                        ...
                        <div>
                          <label className="block text-gray-700 font-medium mb-2">
                            Quantità Confezioni <span className="text-red-500">*</span>
                          </label>
                          <div className="flex items-center space-x-3">
                            ...
                            <button
                              type="button"
                              onClick={() => updateItem(item.id, 'quantity', Math.min(20, item.quantity + 1))}
                              className="w-10 h-10 bg-gray-200 hover:bg-gray-300 rounded-lg flex items-center justify-center transition-colors"
                            >
                      ...
                      {item.type && (
                        <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                          <p className="text-blue-800 font-medium">
                            {item.quantity} confezione{item.quantity > 1 ? 'i' : ''} da {item.format} pezzi - €{(item.format * item.quantity * 0.70).toFixed(2)}
                          </p>
                        </div>
                      )}
                    </div>
                ...
                >
                  ...
                  Aggiungi altro tipo di frollini
                </button>
              </div>
              ...
              <div className="mb-8">
                ...
                <p className="text-sm text-gray-600 mt-2">
                  * I frollini devono essere ordinati con almeno 24 ore di anticipo
                </p>
              </div>
...
export default FrolliniBooking;